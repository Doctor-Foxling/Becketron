...
public:
		...
		: Layer("Example"), m_Camera(-1.6, 1.6f, -0.9f, 0.9f)
	{
		...
		std::shared_ptr<Becketron::VertexBuffer> vertexBuffer;
		...
		std::shared_ptr<Becketron::IndexBuffer> indexBuffer;
		...
		std::shared_ptr<Becketron::VertexBuffer> squareVB;
		...
		std::shared_ptr<Becketron::IndexBuffer> squareIB;
		...
		m_Shader2.reset(Becketron::Shader::CreateShader(vertexSrc2, fragmentSrc2));
	}
	...
	void OnUpdate(Becketron::Timestep ts) override
	{
		...
		for (int y = 0; y < 20; y++)
		{
			for (int x = 0; x < 20; x++)
			{
				...
				Becketron::Renderer::Submit(m_Shader2, m_SquareVA, transform);
			}
		}
...
private:
	std::shared_ptr<Becketron::Shader> m_Shader;
	std::shared_ptr<Becketron::Shader> m_Shader2;   // temp
	std::shared_ptr<Becketron::VertexArray> m_VertexArray;

	std::shared_ptr<Becketron::VertexArray> m_SquareVA;
